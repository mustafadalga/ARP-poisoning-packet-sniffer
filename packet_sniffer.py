...
class Sniffer():
	...
	def get_login_info(self,paket):
		if paket.haslayer(scapy.Raw):
			...
			keywords=["username","user","pass","password","sifre","ad","login","kullanici","kullaniciadi","kullaniciAdi","parola","session_key","session_password"]
	...
	def script_desc(self):
		...
		self.kullanim ="""Kullanim: python packet_sniffer_yedek.py --interface ARAYUZ\nOrnek Kullanim: python packet_sniffer_yedek.py --interface eth0"""
		if sys.version_info[0] >= 3:
			...
			self.description = unicode("Arp Spoofer ile hedef bilgisayar üzerinde ARP zehirlemesi yaparken ,Bu script ile aynı anda http isteklerini dinlemeye alarak olası kullanıcı adı / parola girişlerini görüntüler.","utf8")
